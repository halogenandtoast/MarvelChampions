...
instance HasModifiersFor RocketBoots where
  getModifiersFor _ (IdentityTarget ident) (RocketBoots a) | ident == upgradeController a =
    pure [HitPointModifier 1]
  getModifiersFor _ _ _ = pure []

instance HasAbilities RocketBoots where
  getAbilities (RocketBoots a) = [ability a 1 HeroAction OwnsThis (ExhaustCost <> ResourceCost (Just Mental)) $ RunAbility (toTarget a) 1]

instance RunMessage RocketBoots where
  runMessage msg (RocketBoots attrs) =
    RocketBoots <$> runMessage msg attrs
...
instance RunMessage RocketBootsEffect where
  runMessage msg (RocketBootsEffect attrs) = RocketBootsEffect <$> runMessage msg attrs
